...
-- 계정별 최신 담당자 도출
	...
	SELECT 
			a.cs_seq, IF(a.m_nm='302', a.good_type2, a.m_nm) AS 'm_nm', a.cs_m_id, a.em_seq
	FROM 
			t_contract a,
			(SELECT IF(m_nm='302', good_type2, m_nm) AS 'm_nm', cs_m_id, max(pay_date) AS 'max_pay_date'
			FROM t_contract
			WHERE pay_date BETWEEN '2021-01-01' AND '2022-06-30' AND del_date IS NULL AND sales_type IN ('1') AND agree_state IN ('3')
			GROUP BY IF(m_nm='302', good_type2, m_nm), cs_m_id) b	
	WHERE 
			a.del_date IS NULL 
			AND a.sales_type IN ('1') 
			AND a.agree_state IN ('3') 
	      AND IF(a.m_nm='302', a.good_type2, a.m_nm) = b.m_nm 
			AND a.cs_m_id = b.cs_m_id 
			AND a.pay_date = b.max_pay_date
	      AND em_seq IN (SELECT em_seq FROM em_list)
	GROUP BY a.cs_seq, IF(a.m_nm='302', a.good_type2, a.m_nm), a.cs_m_id, a.em_seq;
...
-- 전기간 월별
	...
	SELECT 
			IF(m_nm='302', good_type2, m_nm) AS 'm_nm', cs_m_id, em_seq, left(pay_date, 7) AS 'pay_month', SUM(pay_price) AS 'pay_price'
	FROM 
			t_contract
	WHERE 
			del_date IS NULL 
	...
	GROUP BY IF(m_nm='302', good_type2, m_nm), cs_m_id, em_seq, left(pay_date, 7);
...
-- 최신 담당자와의 계약 기간(개월수)
	...
	SELECT 
			m_nm, cs_m_id, em_seq, count(pay_month) AS 'count_em'
	FROM 
			month_price
	WHERE 
			(m_nm, cs_m_id, em_seq) IN (SELECT m_nm, cs_m_id, em_seq FROM latest_em) 
			AND pay_month <= '2022-06'
...
-- HM 계약기간(개월수)
	...
	SELECT 
			m_nm, cs_m_id, count(pay_month) AS 'count_id'
	FROM 
			month_price
	WHERE 
			(m_nm, cs_m_id) IN (SELECT m_nm, cs_m_id FROM latest_em) 
			AND pay_month <= '2022-06'
...
-- 날짜 걸러내기
	...
	SELECT 
			m_nm, cs_m_id, em_seq, pay_month, pay_price
	FROM 
			month_price
	WHERE 
			(m_nm, cs_m_id, em_seq) IN (SELECT m_nm, cs_m_id, em_seq FROM latest_em) 
			AND pay_month BETWEEN '2021-01' AND '2022-06';
...
SELECT 
		getcomkrname(te.division1), getEmkrName(le.em_seq), tc.cs_nm, getcomkrname(le.m_nm), le.cs_m_id, ci.count_id, ce.count_em, tc.cs_num, tc.license_cs_status, 
      getcomkrname(tc.cs_type_new2), tc.mg_nm,
      ifnull(concat(tc.mg_tel1, tc.mg_tel2, tc.mg_tel3), '') AS '유선', ifnull(concat(tc.mg_cell1, tc.mg_cell2, tc.mg_cell3), '') AS '무선',
      tc.mg_email, tc.url, mpr.pay_month, mpr.pay_price
